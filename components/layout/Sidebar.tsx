...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const overlayClass = isOpen ? "fixed inset-0 bg-black/50 backdrop-blur-sm z-40 md:hidden" : "hidden"
  const sidebarClass = "fixed top-0 left-0 h-full w-64 bg-white dark:bg-zinc-950 border-r border-zinc-200 dark:border-zinc-800 z-50 transform transition-transform duration-300 ease-in-out md:hidden " + (isOpen ? 'translate-x-0' : '-translate-x-full')
